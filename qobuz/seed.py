...
def get_secret():
    return cc.qobuz_secret

# THANKS TO https://github.com/DashLt/Spoofbuz
class Spoofer:
    def __init__(self):
...
def get_appid_secrets():
    zuboq = Spoofer()
    appid = zuboq.getAppId()
    for tz, secret in zuboq.getSecrets().items():
        print(f'App ID: {appid}   Secret: {secret}   [Timezone: {tz}]')